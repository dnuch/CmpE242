...
static int blinker_242_init(void) {
	
	pr_alert("blinker_242: init\n");
...
static void blinker_242_exit(void) {
	pr_alert("blinker_242: exit\n");
	set_gpio_output(LED_pin_number, GPIO_OUTPUT__LOW);
	iounmap(gpio_registers);
}

module_init(blinker_242_init);
module_exit(blinker_242_exit);
